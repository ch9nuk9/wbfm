...
print(raw_array.shape, preprocessed_array.shape)
...
for file in all_algo_files:
    ...
    print(f'But now only {algo_array.shape}')

    if 'pre' in file:
        final_mask, final_lengths, final_brightnesses = ol.level2_overlap(preprocessed_array, algo_array)
        print(f'- preprocessed: {file}')
    else:
        final_mask, final_lengths, final_brightnesses = ol.level2_overlap(raw_array, algo_array)
    ...
    with open(os.path.join(results_path, os.path.split(file)[1][:-4] + 'lengths_and_brightnesses.pickle'), 'wb') as pickle_out:
        pickle.dump([final_lengths, final_brightnesses], pickle_out)

    # save length histograms
    ol.neuron_length_hist(final_lengths, results_path, 1)
