...
df_traces_gcamp = build_trace_time_series_from_multiple_projects(all_projects_gcamp, use_paper_options=False)
df_traces_gfp = build_trace_time_series_from_multiple_projects(all_projects_gfp, use_paper_options=False)
df_traces_immob = build_trace_time_series_from_multiple_projects(all_projects_immob, use_paper_options=False)
...
df_var_exp_gcamp_melt['has_id'] = df_var_exp_gcamp_melt['neuron_name'].apply(lambda x: 'neuron' not in x and 'VG_' not in x)
...
df_var_exp_gcamp_melt['category'] = df_var_exp_gcamp_melt['has_id'].replace({True: np.nan, False: 'Undetermined'})
...
df_var_exp_gcamp_melt['category'].fillna(tmp, inplace=True)
df_var_exp_gcamp_melt['category'].fillna('Active in Freely Moving only', inplace=True)

df_var_exp_gcamp_melt['marker_size'] = [0.3 if c=='Undetermined' else 1 for c in df_var_exp_gcamp_melt['category']]
