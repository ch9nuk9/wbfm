...
rule invert_curvature_sign:
    ...
    output:
        spline_K_signed = f"{output_behavior_dir}/skeleton_spline_K_signed.csv"
    run:
...
rule fast_fourier_transform:
    input:
        spline_K = f"{output_behavior_dir}/skeleton_spline_K_signed",
    params:
        # project_folder
...
rule annotate_behaviour:
    input:
        curvature_file = f"{output_behavior_dir}/skeleton_spline_K_signed.csv"
...
rule calculate_parameters:
    ...
    input:
        curvature_file = f"{output_behavior_dir}/skeleton_spline_K_signed.csv" #This is used as a parameter because it is only used to find the main dir
    output:
