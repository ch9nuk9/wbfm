...
def example_correlation():
    ...
    def show_hover_panel(get_text_func=None):
        cursor = mplcursors.cursor(**get_nice_cursor_opts())

        if get_text_func:
            # Basic hovering tooltip
            cursor.connect(
                event="add",
                func=lambda sel: sel.annotation.set_text(get_text_func(sel.index)),
            )
    ...
    def on_click(sel):
        ...
        sel.annotation.set_text(on_hover(sel.index))
        print("CLICK")
        print(item)

    show_hover_panel(on_hover)
    plt.show()


def get_nice_cursor_opts():
    return dict(hover = 2,  # Transient
                annotation_kwargs = dict(
                    bbox=dict(
                        boxstyle="square,pad=0.5",
                        facecolor="white",
